...
static void VmxSwitchMTF(hvm_bool enabled)
{
  ...
  /* Log("[HyperDbg] %sable MTF", (enabled == TRUE)?"En":"Dis"); */

  if (enabled == TRUE) {
    /* Enable Monitor Trap Flag */
    CmSetBit32(&v, CPU_BASED_PRIMARY_MTF);
  } else {
    /* Disable Monitor Trap Flag */
...
void VmxHvmInternalHandleExit(void)
{
  ...
  if(vmxcontext.ExitReason == EXIT_REASON_VMCALL || vmxcontext.ExitReason == EXIT_REASON_EPT_MISCONFIGURATION) {// || vmxcontext.ExitReason == EXIT_REASON_MTF || vmxcontext.ExitReason == EXIT_REASON_EPT_VIOLATION) {
    HandlerLogging = TRUE;
  } else {
  ...
  case EXIT_REASON_MTF:
    ...
    /* MmuReadVirtualRegion(context.GuestContext.cr3, context.GuestContext.rip, &curr_opcode1, sizeof(curr_opcode1)); */
    /* MmuReadVirtualRegion(context.GuestContext.cr3, context.GuestContext.rip+4, &curr_opcode2, sizeof(curr_opcode2)); */

    /* Log("MTF @ 0x%08hx", context.GuestContext.rip); */
    /* ComPrint("###Opcode: \\x%02x\\x%02x\\x%02x\\x%02x", (curr_opcode1 >>  0) & 0xff, */
    /* 	                                                (curr_opcode1 >>  8) & 0xff, */
    /* 	                                                (curr_opcode1 >> 16) & 0xff, */
    /*                                                     (curr_opcode1 >> 24) & 0xff); */
    /* ComPrint("\\x%02x\\x%02x\\x%02x\\x%02x\n",          (curr_opcode2 >>  0) & 0xff, */
    /* 	                                                (curr_opcode2 >>  8) & 0xff, */
    /*                                                     (curr_opcode2 >> 16) & 0xff, */
    /*                                                     (curr_opcode2 >> 24) & 0xff); */
    HandleMTF();
  ...
  case EXIT_REASON_EPT_VIOLATION:
    ...
    /* Log("Violation at va 0x%08hx, fail on phy 0x%08hx", vmxcontext.GuestLinearAddress, GET32L(vmxcontext.GuestPhysicalAddress)); */
    /* ComPrint("###Opcode: \\x%02x\\x%02x\\x%02x\\x%02x", (curr_opcode1 >>  0) & 0xff, */
    /* 	                                                (curr_opcode1 >>  8) & 0xff, */
    /* 	                                                (curr_opcode1 >> 16) & 0xff, */
    /*                                                     (curr_opcode1 >> 24) & 0xff); */
    ...
    /* Log("[EIP: 0x%08hx] GuestLinear %s valid, attempt type %x, violation type = %s, %s", context.GuestContext.rip, (vmxcontext.ExitQualification & 0x80) != 0?"is":"is !NOT!", vmxcontext.ExitQualification & 0x7, ((vmxcontext.ExitQualification & 0x100) != 0)?"normal":"in page walk", (vmxcontext.ExitQualification & 0x38) == 0?"fill an entry...":"ACCESS VIOLATION !!!"); */
 ...
 Resume:
  ...
  /* PTE pte; */
  /* hvm_bool isLarge; */
  /* if(context.GuestContext.rsi == 0x85a1f160) { */

  /*   MmuGetPageEntry(context.GuestContext.cr3, context.GuestContext.rsi, &pte, &isLarge); */
